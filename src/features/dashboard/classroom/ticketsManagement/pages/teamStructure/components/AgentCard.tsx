...
export function AgentCard({ agent }: AgentCardProps) {
    ...
    const statusLabel = {
        available: t("teamStructure.status.available", "Available"),
        busy: t("teamStructure.status.busy", "Busy"),
        offline: t("teamStructure.status.offline", "Offline"),
    };

    const roleLabel = {
        lead: t("teamStructure.role.lead", "Lead"),
        agent: t("teamStructure.role.agent", "Agent"),
    };
        ...
        >
            ...
            <div className="flex-1 min-w-0 flex flex-col gap-1">
                ...
                <p className="text-[12px] text-gray-900/80 dark:text-gray-400">
                    {agent.activeTickets}{" "}
                    {t("teamStructure.activeTickets", "active tickets")}
                </p>
            </div>
        </div>
    );
