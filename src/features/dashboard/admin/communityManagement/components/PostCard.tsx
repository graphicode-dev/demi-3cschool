...
import { 
  ...
  Globe, CornerDownRight, Pin, 
  Trash2, Edit3, Link2, ThumbsUp, Laugh, Zap, Star,
  ShieldCheck, CheckCircle2, Flag, BookOpen, Check,
  AtSign, BarChart3
} from 'lucide-react';
...
export function PostCard({ 
  ...
  const categoryColors: Record<string, string> = {
    'Project Help': 'bg-red-50 text-red-600 border-red-100 dark:bg-red-900/20 dark:text-red-400 dark:border-red-800',
    'Homework': 'bg-amber-50 text-amber-600 border-amber-100 dark:bg-amber-900/20 dark:text-amber-400 dark:border-amber-800',
    'Coding Tip': 'bg-emerald-50 text-emerald-600 border-emerald-100 dark:bg-emerald-900/20 dark:text-emerald-400 dark:border-emerald-800',
    'Resource': 'bg-indigo-50 text-indigo-600 border-indigo-100 dark:bg-indigo-900/20 dark:text-indigo-400 dark:border-indigo-800',
  };
  ...
  const CommentItem = ({ comment, isReply = false }: { comment: Comment; isReply?: boolean }) => {
    return (
      <div className={`flex gap-3 ${isReply ? 'ml-10 mt-3' : 'mt-5'}`}>
        {isReply && <CornerDownRight size={14} className="text-gray-300 dark:text-gray-600 mt-2 shrink-0" />}
        <img src={comment.author.avatar} alt="" className="w-9 h-9 rounded-xl shrink-0 border border-gray-100 dark:border-gray-700 shadow-sm" />
        <div className="flex-1">
          <div className={`bg-white dark:bg-gray-800 border rounded-2xl p-4 shadow-sm relative ${comment.isSolution ? 'border-emerald-500 ring-2 ring-emerald-50 dark:ring-emerald-900/30' : 'border-gray-100 dark:border-gray-700'}`}>
            {comment.isSolution && (
            ...
            <div className="flex items-center justify-between mb-1 gap-4">
              <div className="flex items-center gap-1.5">
                <p className="text-[12px] font-bold text-gray-800 dark:text-gray-200">{comment.author.name}</p>
              ...
              <p className="text-[10px] text-gray-400 dark:text-gray-500 font-medium">{comment.createdAt}</p>
            </div>
            <p className="text-[13px] text-gray-600 dark:text-gray-300 leading-relaxed">{comment.content}</p>
          </div>
  ...
  return (
    <div className={`bg-white dark:bg-gray-800 rounded-[32px] border border-gray-100 dark:border-gray-700 shadow-sm overflow-hidden mb-8 transition-all hover:shadow-lg ${post.isPinned ? 'ring-2 ring-[#00ADEF]/20' : ''} ${post.isOfficial ? 'border-l-8 border-l-[#00ADEF]' : ''}`}>
      {post.isPinned && (
      ...
      {post.category && post.category !== 'General' && (
        <div className={`px-8 py-3 flex items-center justify-between border-b ${categoryColors[post.category]}`}>
          ...
          {post.status && (
            <span className={`text-[9px] font-black px-3 py-1 rounded-full border uppercase tracking-tighter ${post.status === 'Solved' ? 'bg-emerald-500 text-white border-emerald-600' : 'bg-white dark:bg-gray-700 text-current border-current'}`}>
              {post.status}
            </span>
          )}
        </div>
      ...
      <div className="p-8">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-4">
            <div className="relative">
               <img src={post.author.avatar} alt="" className="w-12 h-12 rounded-2xl object-cover ring-4 ring-gray-50 dark:ring-gray-700 border border-gray-100 dark:border-gray-600" />
            ...
            <div>
              <div className="flex items-center gap-2">
                <p className="text-[15px] font-black text-gray-800 dark:text-gray-100 tracking-tight">
                  {post.author.name}
                  {post.feeling && <span className="text-gray-400 dark:text-gray-500 font-medium ml-1">is feeling {post.feeling}</span>}
                </p>
                {(post.author.role === 'manager' || post.isOfficial) && <CheckCircle2 size={16} className="text-[#00ADEF]" />}
                {post.author.role === 'instructor' && <span className="text-[9px] bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 px-1.5 py-0.5 rounded font-black uppercase">Teacher</span>}
              </div>
              <div className="flex items-center gap-2 text-[11px] text-gray-400 dark:text-gray-500 font-medium">
                ...
                <span className="w-1 h-1 bg-gray-300 dark:bg-gray-600 rounded-full"></span>
                <Globe size={11} className="text-gray-300 dark:text-gray-600" />
              </div>
            </div>
          </div>
          <div className="relative">
            <button onClick={() => setShowMenu(!showMenu)} className="text-gray-300 dark:text-gray-600 hover:text-gray-500 dark:hover:text-gray-400 p-2.5 hover:bg-gray-50 dark:hover:bg-gray-700 rounded-xl transition-all">
            ...
            {showMenu && (
              <div className="absolute right-0 mt-2 w-52 bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 shadow-2xl rounded-2xl z-50 overflow-hidden py-1.5 animate-in zoom-in-95">
                {!post.category || post.category === 'General' ? (
                   <button onClick={() => { setIsEditing(true); setShowMenu(false); }} className="w-full flex items-center gap-3 px-4 py-2.5 text-[13px] text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                    <Edit3 size={16} /> Edit
                  </button>
                ) : null}
                <button onClick={copyLink} className="w-full flex items-center gap-3 px-4 py-2.5 text-[13px] text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                  <Link2 size={16} /> Copy Link
                </button>
                <button onClick={handleReport} className="w-full flex items-center gap-3 px-4 py-2.5 text-[13px] text-orange-500 hover:bg-orange-50 dark:hover:bg-orange-900/20 transition-colors">
                  <Flag size={16} /> Report
                </button>
                <div className="h-px bg-gray-50 dark:bg-gray-700 my-1.5 mx-2"></div>
                <button onClick={() => { onDelete(post.id); setShowMenu(false); }} className="w-full flex items-center gap-3 px-4 py-2.5 text-[13px] text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors">
              ...
              {post.taggedUsers && post.taggedUsers.length > 0 && (
                <div className="flex flex-wrap gap-2 mt-4">
                  {post.taggedUsers.map(u => (
                    <span key={u.id} className="inline-flex items-center gap-1 text-[12px] font-bold text-[#00ADEF] bg-blue-50 dark:bg-blue-900/30 px-2.5 py-1 rounded-lg border border-blue-100 dark:border-blue-800">
                ...
                return (
                  ...
                  >
                    ...
                    <div className="absolute inset-0 flex items-center justify-between px-5">
                      <span className="text-[13px] font-bold text-gray-700 dark:text-gray-200">{opt.text}</span>
        ...
        <div className="flex items-center justify-between pt-6 border-t border-gray-50 dark:border-gray-700">
          <div className="flex items-center gap-6">
            ...
            <button onClick={() => setShowComments(!showComments)} className="flex items-center gap-2 text-[#64748b] dark:text-gray-400 hover:text-indigo-600 transition-all font-bold text-[13px]">
            ...
            <button onClick={copyLink} className="flex items-center gap-2 text-[#64748b] dark:text-gray-400 hover:text-blue-500 transition-all font-bold text-[13px]">
              <Link2 size={18} />
              <span className="hidden sm:inline">Link</span>
            </button>
          </div>
          <div className="flex items-center gap-2">
            <button onClick={() => onPin(post.id)} className={`p-2.5 rounded-xl transition-all ${post.isPinned ? 'text-[#00ADEF] bg-[#E0F4FF] dark:bg-[#00ADEF]/20' : 'text-gray-300 dark:text-gray-600 hover:text-[#00ADEF] hover:bg-gray-50 dark:hover:bg-gray-700'}`}>
              <Pin size={18} fill={post.isPinned ? 'currentColor' : 'none'} />
            </button>
            <button onClick={() => onSave(post.id)} className={`p-2.5 rounded-xl transition-all ${post.isSaved ? 'text-[#00ADEF] bg-[#E0F4FF] dark:bg-[#00ADEF]/20' : 'text-gray-300 dark:text-gray-600 hover:text-[#00ADEF] hover:bg-gray-50 dark:hover:bg-gray-700'}`}>
        ...
        {showComments && (
          <div className="mt-8 space-y-4 pt-6 bg-[#fcfdfe] dark:bg-gray-700/30 border-t border-gray-50 dark:border-gray-700 rounded-[40px] px-8 pb-8 -mx-8 -mb-8 animate-in slide-in-from-bottom-4 duration-300">
            <div className="flex items-center justify-between mb-4">
              <h5 className="text-[11px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-widest">Discussion Thread</h5>
              <div className="flex gap-4">
                <button onClick={() => setCommentSort('newest')} className={`text-[10px] font-black uppercase tracking-tight ${commentSort === 'newest' ? 'text-[#00ADEF]' : 'text-gray-300 dark:text-gray-600'}`}>Newest</button>
                <button onClick={() => setCommentSort('top')} className={`text-[10px] font-black uppercase tracking-tight ${commentSort === 'top' ? 'text-[#00ADEF]' : 'text-gray-300 dark:text-gray-600'}`}>Top Rated</button>
              </div>
            </div>
            ...
            {sortedComments.length > 0 ? (
              ...
              <p className="text-center py-6 text-[13px] text-gray-400 dark:text-gray-500 font-medium">No comments yet. Start the conversation!</p>
            )}

            <div className="flex gap-4 mt-8 pt-6 border-t border-gray-100 dark:border-gray-600">
              <img src={CURRENT_USER.avatar} alt="" className="w-10 h-10 rounded-xl shadow-sm border border-white dark:border-gray-700" />
              <div className="flex-1 relative">
                <input
                  type="text"
                  placeholder="Add a comment..."
                  className="w-full bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 rounded-2xl py-3 px-5 text-[14px] text-gray-800 dark:text-gray-200 outline-none focus:ring-2 focus:ring-[#00ADEF]/10 focus:border-[#00ADEF] transition-all shadow-sm"
                />
              </div>
            </div>
          </div>
