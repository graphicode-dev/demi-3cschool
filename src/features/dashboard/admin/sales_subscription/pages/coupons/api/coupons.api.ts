...
import { mockCoupons, mockCouponUsages, generateMockCode, mockValidateResponse } from "./mock";
...
export const couponsApi = {
    ...
    getList: async (
        ...
        if (USE_MOCK_API) {
            return new Promise((resolve) => setTimeout(() => resolve([...mockCoupons]), 500));
        }
    ...
    getById: async (id: string, signal?: AbortSignal): Promise<Coupon> => {
        if (USE_MOCK_API) {
            const mock = mockCoupons.find((c) => c.id === Number(id));
            if (!mock) throw new Error("Coupon not found");
    ...
    getUsages: async (
        ...
        if (USE_MOCK_API) {
            return new Promise((resolve) => setTimeout(() => resolve([...mockCouponUsages]), 500));
        }
    ...
    create: async (payload: CouponCreatePayload): Promise<Coupon> => {
        if (USE_MOCK_API) {
            const newCoupon: Coupon = {
                ...
                type: payload.type,
                typeLabel: payload.type === "percentage" ? "Percentage" : "Fixed",
                value: payload.value.toString(),
                status: payload.status || "active",
                statusLabel: payload.status === "active" ? "Active" : "Inactive",
                createdAt: new Date().toISOString(),
                updatedAt: new Date().toISOString()
            };
            return new Promise((resolve) => setTimeout(() => resolve(newCoupon), 500));
        }
    ...
    update: async (
        ...
        if (USE_MOCK_API) {
            const mock = mockCoupons.find((c) => c.id === Number(id)) || mockCoupons[0];
            const updated: Coupon = { ...mock, ...payload, updatedAt: new Date().toISOString() } as unknown as Coupon;
    ...
    generateCode: async (
        ...
        if (USE_MOCK_API) {
            return new Promise((resolve) => setTimeout(() => resolve(generateMockCode()), 500));
        }
    ...
    validate: async (
        ...
        if (USE_MOCK_API) {
            return new Promise((resolve) => setTimeout(() => resolve(mockValidateResponse), 500));
        }
    ...
    changeStatus: async (
        ...
        if (USE_MOCK_API) {
            const mock = mockCoupons.find((c) => c.id === Number(id)) || mockCoupons[0];
            const updated: Coupon = { ...mock, status: payload.status, updatedAt: new Date().toISOString() };
            return new Promise((resolve) => setTimeout(() => resolve(updated), 500));
        }
