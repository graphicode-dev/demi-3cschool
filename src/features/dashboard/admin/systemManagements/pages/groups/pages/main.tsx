...
import type { Group } from "@/features/dashboard/admin/groupsManagement/types/groups.types";
import { Teacher, useTeachersList } from "../../users/api";
import type { Student } from "@/features/dashboard/admin/systemManagements/types";
...
function GroupDetailsView({
    ...
    const styles = getLevelStyles(group.level.title);

    const [isDropdownOpen, setIsDropdownOpen] = useState(false);
    const [instSearch, setInstSearch] = useState("");
    const [pendingInstructor, setPendingInstructor] = useState<Teacher | null>(
        null
    );
    ...
    const handleConfirmInstructor = () => {
        if (pendingInstructor) {
            onReassignInstructor(Number(group.id), pendingInstructor.id);
            setPendingInstructor(null);
            setIsDropdownOpen(false);
        }
    };

    const formatSchedule = (schedules: Group["schedules"]) => {
        if (!schedules || schedules.length === 0) return "—";
        return schedules
            .map((s) => `${s.dayOfWeek.slice(0, 3)} ${s.startTime}`)
            .join(", ");
    };
                        ...
                        <div className="bg-white dark:bg-gray-900 p-6 rounded-3xl shadow-sm border border-gray-100 dark:border-gray-700 flex flex-col transition-all hover:shadow-md">
                            <div className="flex items-center justify-between mb-5">
                                ...
                                <div className="flex gap-2">
                                    <button
                                        onClick={() => openWhatsApp(undefined)}
                                                        ...
                                                        filteredInstructors.map(
                                                            (inst) => (
                                                                <button
                                                                    ...
                                                                    className={`w-full flex items-center gap-4 px-5 py-3.5 transition-all hover:bg-blue-50/50 dark:hover:bg-blue-500/5 text-start group/item ${inst.id === instructor?.id ? "bg-blue-50/30 dark:bg-blue-500/5" : ""}`}
                                                                >
                                                                    ...
                                                                    <div className="flex-1 min-w-0">
                                                                        <p
                                                                            className={`text-[13px] font-bold truncate leading-none mb-1 ${inst.id === instructor?.id ? "text-brand-500" : "text-gray-800 dark:text-gray-200"}`}
                                                                        >
                                                                            {
                                                                                inst.name
                                                                            }
...
function GroupsManagement() {
    ...
    const { data: groupsData, isLoading } = useGroupsList({
        page,
        search: searchQuery || undefined,
    });

    const currentPage = groupsData?.currentPage ?? 1;
    const totalPages = groupsData?.lastPage ?? 1;
    const perPage = groupsData?.perPage ?? 10;
    const totalItems = totalPages * perPage;
    const { data: teachersData } = useTeachersList();
    const { data: selectedGroupData } = useGroup(selectedGroupId);
    const assignBlocksMutation = useAssignBlocks();
    const reassignInstructorMutation = useReassignInstructor();

    const filteredGroups = groupsData?.items ?? [];
    const allInstructors: Teacher[] = teachersData ?? [];

    const selectedGroup = selectedGroupData ?? null;

    const handleOpenAssignModal = (group: Group) => {
        setTempSelectedBlocks((group.squads ?? []).map((sq) => sq.id));
        setShowAssignModal(Number(group.id));
    };
    ...
    const handleConfirmAssign = (groupId: number) => {
        assignBlocksMutation.mutate(
            { groupId, payload: { block_ids: tempSelectedBlocks } },
    ...
    const handleReassignInstructor = (
        ...
        reassignInstructorMutation.mutate({ groupId, instructorId });
    };
    ...
    return (
        <PageWrapper
            pageHeaderProps={{
                ...
                actions: (
                    <button
                        // onClick={() => navigate("/admin/groups/grades")}
                        className="inline-flex items-center gap-2.5 px-8 py-4 bg-linear-to-r from-brand-400 via-blue-500 to-indigo-600 text-white rounded-2xl font-bold text-[11px] uppercase tracking-widest shadow-xl hover:shadow-2xl hover:scale-[1.02] transition-all"
                    >
            ...
            {!isLoading && (
                <div className="grid grid-cols-[repeat(auto-fill,minmax(320px,1fr))] gap-6 items-stretch">
                    {filteredGroups.map((group, idx) => {
                        const instructor =
                            group.instructor ?? group.primaryTeacher;
                        const assignedSquads = group.squads ?? [];
                        const styles = getLevelStyles(group.level.title);
                        ...
                        return (
                            ...
                            >
                                ...
                                {showAssignModal === Number(group.id) && (
