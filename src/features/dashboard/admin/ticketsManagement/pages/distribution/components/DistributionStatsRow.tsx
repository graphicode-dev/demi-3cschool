/**
 * DistributionStatsRow Component
 *
 * Displays a row of stat cards for the distribution page.
 * Matches Figma design node 1264:13031
 */
...
interface DistributionStatsRowProps {
...
export function DistributionStatsRow({
    stats,
    isLoading,
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            <StatItem
                ...
                label={t(
                    "distribution.stats.totalActiveTickets",
            ...
            <StatItem
                ...
                label={t(
                    "distribution.stats.availableAgents",
                    "Available Agents"
                )}
            />
            <StatItem
                icon={<Scale className="w-[18px] h-[18px]" />}
                value={stats.avgTicketsPerAgent.toFixed(1)}
                label={t(
                    "distribution.stats.avgTicketsPerAgent",
                    "Avg Tickets/Agent"
                )}
            />
        </div>
...
export default DistributionStatsRow;
