 ...
 *
 * Displays a row of stat cards for the team structure page.
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            <StatItem
                ...
                label={t("teamStructure.stats.totalBlocks", "Total Blocks")}
            />
            <StatItem
                icon={<Crown className="w-[18px] h-[18px]" />}
                value={stats.blockLeads}
                label={t("teamStructure.stats.blockLeads", "Block Leads")}
            />
            <StatItem
                icon={<User className="w-[18px] h-[18px]" />}
                value={stats.totalAgents}
                label={t("teamStructure.stats.totalAgents", "Total Agents")}
            />
        </div>
    );
}
